...
export default class Main {
  ...
  floorGenerate() {
    ...
    if (databus.frame % 80 === 0) {
      var index = Math.floor(Math.random() * this.floorType.length);

      if (index === databus.lastFloorIndex) {
        if (index === this.floorType.length - 1) {
          index = 0;
        } else {
          index = index + 1;
        }
      }
      databus.lastFloorIndex = index;
  ...
  loop() {
    ...
    this.aniId = window.requestAnimationFrame(this.bindLoop);
    databus.now = Date.now();
    databus.delta = databus.now - databus.then;
　　if (databus.delta > databus.interval) {
  　　databus.then = databus.now - (databus.delta % databus.interval);
　　　databus.frame++;
      this.update();
      this.render();
　　}  
